...
description: 在开始需要与当前工作空间隔离的功能工作，或在执行实现计划之前使用 - 创建带有智能目录选择和安全验证的隔离 git worktrees
---
...
**开始时宣布：** "我正在使用 using-git-worktrees skill 来设置隔离工作空间。"
...
- **修复：** 总是使用 `git check-ignore` 创建项目本地 worktree 前
...
```
你：我正在使用 using-git-worktrees skill 来设置隔离工作空间。
...
**被调用：**
- **brainstorming**（阶段 4）- 设计批准且后续实现时必需
